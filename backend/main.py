...
app.include_router(customer.router, prefix="/customer", tags=["Customer"], dependencies=[Depends(require_user)])
app.include_router(ticket_routes.router, prefix="/customer", dependencies=[Depends(require_user)])
...
@app.get("/customer/complaints/")
def get_customer_complaints(db: Session = Depends(get_db), user=Depends(require_user)):
    return db.query(Complaint).all()
