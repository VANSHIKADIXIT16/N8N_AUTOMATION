...
export default function CustomerServiceDashboard() {
  ...
  const fetchTickets = async () => {
    setLoading(true);
    try {
      const response = await auth.fetchWithAuth(`/tickets/assigned/${agent_id}`);
  ...
  const handleResolve = async (ticketId: number) => {
    try {
      const response = await auth.fetchWithAuth(`/tickets/${ticketId}/resolve`, {
        method: "PATCH",
      });
      if (response.ok) {
        toast({
  ...
  const handleCreateTicket = async () => {
    try {
      const response = await auth.fetchWithAuth("/tickets/", {
        method: "POST",
