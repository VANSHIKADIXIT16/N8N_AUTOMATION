...
export default function CustomerServiceDashboard() {
  ...
  const fetchTickets = async () => {
    setLoading(true);
    try {
      const response = await fetch(`http://localhost:8000/customer/tickets/assigned/${agent_id}`, {
        headers: {
          ...auth.getAuthHeader(),
        },
      });
  ...
  const handleResolve = async (ticketId: number) => {
    try {
      const response = await fetch(`http://localhost:8000/customer/tickets/${ticketId}/resolve`, {
        method: "PATCH",
        headers: {
          ...auth.getAuthHeader(),
        },
  ...
  const handleCreateTicket = async () => {
    try {
      const response = await fetch("http://localhost:8000/customer/tickets/", {
        method: "POST",
